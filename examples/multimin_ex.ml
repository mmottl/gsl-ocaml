...
let parab a b =
  ...
  let fdf ~x ~g =
    ...
    (10. *. xa *. xa) +. (20. *. yb *. yb) +. 30.
  in
  { multim_f = f; multim_df = df; multim_fdf = fdf }
...
let mini kind gf start ~step ~tol =
  let minim =
    Multimin.Deriv.make kind 2 gf ~x:(Vector.of_array start) ~step ~tol
  in
...
let mini_simplex kind gf ~start ~step_size =
  let minim =
    Multimin.NoDeriv.make kind 2 gf ~x:(Vector.of_array start)
      ~step_size:(Vector.of_array step_size)
  in
...
let _ =
  ...
  mini_simplex Multimin.NoDeriv.NM_SIMPLEX (parab 1. 2.).multim_f
    ~start:[| 5.; 7. |] ~step_size:[| 1.; 1. |]
