...
CAMLprim value ml_gsl_stats_mean(value ow, value data) {
  ...
  if (Is_none(ow))
    ...
    result =
        gsl_stats_wmean(Double_array_val(w), 1, Double_array_val(data), 1, len);
  }
...
CAMLprim value ml_gsl_stats_variance(value ow, value omean, value data) {
  ...
  if (Is_none(ow))
    ...
    if (Is_none(omean))
      ...
      result =
          gsl_stats_wvariance_m(Double_array_val(w), 1, Double_array_val(data),
                                1, len, Double_val(Unoption(omean)));
  }
  return caml_copy_double(result);
}
...
CAMLprim value ml_gsl_stats_sd(value ow, value omean, value data) {
  ...
  if (Is_none(ow))
    ...
    if (Is_none(omean))
      result =
          gsl_stats_wsd(Double_array_val(w), 1, Double_array_val(data), 1, len);
    else
      result = gsl_stats_wsd_m(Double_array_val(w), 1, Double_array_val(data),
                               1, len, Double_val(Unoption(omean)));
  }
  return caml_copy_double(result);
}

CAMLprim value ml_gsl_stats_variance_with_fixed_mean(value ow, value mean,
                                                     value data) {
  size_t len = Double_array_length(data);
  double result;
  if (Is_none(ow))
    result = gsl_stats_variance_with_fixed_mean(Double_array_val(data), 1, len,
                                                Double_val(mean));
  else {
    value w = Unoption(ow);
    check_array_size(data, w);
...
CAMLprim value ml_gsl_stats_sd_with_fixed_mean(value ow, value mean,
                                               value data) {
  size_t len = Double_array_length(data);
  double result;
  if (Is_none(ow))
    result = gsl_stats_sd_with_fixed_mean(Double_array_val(data), 1, len,
                                          Double_val(mean));
  else {
    value w = Unoption(ow);
    check_array_size(data, w);
...
CAMLprim value ml_gsl_stats_absdev(value ow, value omean, value data) {
  ...
  if (Is_none(ow))
    ...
    check_array_size(data, w);
    if (Is_none(omean))
      result = gsl_stats_wabsdev(Double_array_val(w), 1, Double_array_val(data),
                                 1, len);
    else
      result =
          gsl_stats_wabsdev_m(Double_array_val(w), 1, Double_array_val(data), 1,
                              len, Double_val(Unoption(omean)));
  }
  return caml_copy_double(result);
}
...
CAMLprim value ml_gsl_stats_skew(value ow, value data) {
  ...
  if (Is_none(ow))
    ...
    check_array_size(data, w);
    result =
        gsl_stats_wskew(Double_array_val(w), 1, Double_array_val(data), 1, len);
  }
  return caml_copy_double(result);
}

CAMLprim value ml_gsl_stats_skew_m_sd(value ow, value mean, value sd,
                                      value data) {
  size_t len = Double_array_length(data);
  double result;
  if (Is_none(ow))
    ...
    check_array_size(data, w);
    result =
        gsl_stats_wskew_m_sd(Double_array_val(w), 1, Double_array_val(data), 1,
                             len, Double_val(mean), Double_val(sd));
  }
  return caml_copy_double(result);
}
...
CAMLprim value ml_gsl_stats_kurtosis(value ow, value data) {
  ...
  if (Is_none(ow))
    ...
    value w = Unoption(ow);
    check_array_size(data, w);
    result = gsl_stats_wkurtosis(Double_array_val(w), 1, Double_array_val(data),
                                 1, len);
  }
  return caml_copy_double(result);
}

CAMLprim value ml_gsl_stats_kurtosis_m_sd(value ow, value mean, value sd,
                                          value data) {
  size_t len = Double_array_length(data);
  double result;
  if (Is_none(ow))
    ...
    check_array_size(data, w);
    result =
        gsl_stats_wkurtosis_m_sd(Double_array_val(w), 1, Double_array_val(data),
                                 1, len, Double_val(mean), Double_val(sd));
  }
  return caml_copy_double(result);
}
...
CAMLprim value ml_gsl_stats_covariance_m(value mean1, value data1, value mean2,
                                         value data2) {
  size_t len = Double_array_length(data1);
  double result;
  check_array_size(data1, data2);
...
CAMLprim value ml_gsl_stats_quantile_from_sorted_data(value data, value f) {
  size_t len = Double_array_length(data);
  double r = gsl_stats_quantile_from_sorted_data(Double_array_val(data), 1, len,
                                                 Double_val(f));
...
CAMLprim value ml_gsl_stats_correlation(value data1, value data2) {
  ...
  double r;
  check_array_size(data1, data2);
  r = gsl_stats_correlation(Double_array_val(data1), 1, Double_array_val(data2),
                            1, len);
  return caml_copy_double(r);
}
