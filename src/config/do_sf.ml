...
let split ?(collapse = false) c s =
  ...
  let rec proc accu n =
    let n' =
      match String.index_from s n c with i -> i | exception Not_found -> len
    in
    let accu' =
      if collapse && n' = n then accu else String.sub s n (n' - n) :: accu
    in
    if n' >= len - 1 then List.rev accu' else proc accu' (n' + 1)
  in
  proc [] 0

let words_list s = split ~collapse:true ' ' s

(** Quotation for externals : << fun1,arg1,arg2 >> -> external fun1 : arg1 ->
    arg2 = "fun1" << fun1@fun_c,arg1,arg2 >> -> external fun1 : arg1 -> arg2 =
    "fun_c" << fun1@fun_c@fun_f,float,float >> -> external fun1 : float -> float
    = "fun_c" "fun_f" "float" *)
let ext_quot =
  ...
  fun str ->
    ...
    | name_r :: (arg1 :: argr as args) ->
        ...
        in
        Format.fprintf bh "@[<2>external %s : %s" (String.trim name)
          (String.trim arg1);
  ...
  | "cyl" :: letter :: tl ->
      ...
      String.concat ""
        [
          ...
          ext_quot
            (sprintf
               "bessel_%sn_array@ml_gsl_sf_bessel_%sn_array,int,float,float \
                array,unit"
               letter letter);
        ]
  | "cyl_scaled" :: letter :: tl ->
      ...
      String.concat ""
        [
          ...
          ext_quot
            (sprintf
               "bessel_%sn_scaled_array@ml_gsl_sf_bessel_%sn_scaled_array,int,float,float \
                array,unit"
               letter letter);
        ]
  | "sph" :: letter :: tl ->
      ...
      String.concat ""
        [
          ...
          ext_quot
            (sprintf
               "bessel_%sl_array@ml_gsl_sf_bessel_%sl_array,int,float,float \
                array,unit"
               letter letter);
        ]
  | "sph_scaled" :: letter :: tl ->
      ...
      String.concat ""
        [
          ...
          ext_quot
            (sprintf
               "bessel_%sl_scaled_array@ml_gsl_sf_bessel_%sl_scaled_array,int,float,float \
                array,unit"
               letter letter);
        ]
...
let process_line =
  ...
  fun l ->
    if not (Str.string_match quotation l 0) then l
      ...
      let quot = try Str.matched_group 1 l with Not_found -> ":sf" in
