...
static int ml_gsl_odeiv_func(double t, const double y[], double dydt[],
                             void *params) {
...
static int ml_gsl_odeiv_jacobian(double t, const double y[], double *dfdy,
...
CAMLprim value ml_gsl_odeiv_alloc_system(value func, value ojac, value dim) {
  ...
  p->mat = Is_none(ojac) ? Val_unit
                         : caml_ba_alloc_dims(barr_flags, 2, NULL, Int_val(dim),
                                              Int_val(dim));
...
CAMLprim value ml_gsl_odeiv_step_alloc(value step_type, value dim) {
  const gsl_odeiv_step_type *steppers[] = {
      ...
      gsl_odeiv_step_rk2simp, gsl_odeiv_step_rk4imp, gsl_odeiv_step_bsimp,
      gsl_odeiv_step_gear1,   gsl_odeiv_step_gear2,
  };
  gsl_odeiv_step *step =
      gsl_odeiv_step_alloc(steppers[Int_val(step_type)], Int_val(dim));
  value res;
...
CAMLprim value ml_gsl_odeiv_step_apply(value step, value t, value h, value y,
                                       value yerr, value odydt_in,
                                       value odydt_out, value syst) {
  ...
  if (len_dydt_in)
    memcpy(dydt_in, Double_array_val(Unoption(odydt_in)),
           Bosize_val(Unoption(odydt_in)));
  ...
  status = gsl_odeiv_step_apply(
      ODEIV_STEP_VAL(step), Double_val(t), Double_val(h), y_copy, yerr_copy,
      len_dydt_in ? dydt_in : NULL, len_dydt_out ? dydt_out : NULL,
  ...
  if (len_dydt_out)
    memcpy(Double_array_val(Unoption(odydt_out)), dydt_out,
           Bosize_val(Unoption(odydt_out)));

  CAMLreturn(Val_unit);
}

CAMLprim value ml_gsl_odeiv_step_apply_bc(value argv[], int argc) {
  return ml_gsl_odeiv_step_apply(argv[0], argv[1], argv[2], argv[3], argv[4],
...
CAMLprim value ml_gsl_odeiv_control_scaled_new(value eps_abs, value eps_rel,
                                               value a_y, value a_dydt,
                                               value scale_abs) {
  gsl_odeiv_control *c = gsl_odeiv_control_scaled_new(
      Double_val(eps_abs), Double_val(eps_rel), Double_val(a_y),
      Double_val(a_dydt), Double_array_val(scale_abs),
...
CAMLprim value ml_gsl_odeiv_control_hadjust(value c, value s, value y,
  ...
  int status = gsl_odeiv_control_hadjust(
      ODEIV_CONTROL_VAL(c), ODEIV_STEP_VAL(s), Double_array_val(y),
      Double_array_val(yerr), Double_array_val(dydt), &c_h);
  {
    CAMLparam0();
    CAMLlocal2(vh, r);
...
CAMLprim value ml_gsl_odeiv_control_hadjust_bc(value *argv, int argc) {
  return ml_gsl_odeiv_control_hadjust(argv[0], argv[1], argv[2], argv[3],
                                      argv[4], argv[5]);
}
...
CAMLprim value ml_gsl_odeiv_evolve_apply(value e, value c, value s, value syst,
                                         value t, value t1, value h, value y) {
...
CAMLprim value ml_gsl_odeiv_evolve_apply_bc(value *argv, int argc) {
  return ml_gsl_odeiv_evolve_apply(argv[0], argv[1], argv[2], argv[3], argv[4],
                                   argv[5], argv[6], argv[7]);
}
