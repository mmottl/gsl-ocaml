...
CAMLprim value ml_gsl_multiroot_fdfsolver_set(value S, value fun, value X) {
  ...
  if (v_X.size != p->gslfun.mrfdf.n)
    GSL_ERROR("wrong number of dimensions for function", GSL_EBADLEN);
  gsl_multiroot_fdfsolver_set(GSLMULTIROOTFDFSOLVER_VAL(S), &(p->gslfun.mrfdf),
                              &v_X);
  CAMLreturn(Val_unit);
}
...
CAMLprim value ml_gsl_multiroot_fsolver_get_state(value S, value ox, value of,
                                                  value odx, value unit) {
...
CAMLprim value ml_gsl_multiroot_test_delta_f(value S, value epsabs,
                                             value epsrel) {
  int status;
...
CAMLprim value ml_gsl_multiroot_test_delta_fdf(value S, value epsabs,
                                               value epsrel) {
  int status;
